...
`.claude/skills/` 에 이 저장소 전용 워크플로우가 있습니다. `/이름` 으로 호출합니다.

| 스킬 | 언제 쓰나 |
|---|---|
| `/git` | 커밋·브랜치·이슈·PR 생성. `docs/01-conventions.md` 의 규칙을 그대로 따릅니다 |
| `/planmd` | 개발 전 구현 계획을 `docs/plans/` 에 문서로 뽑습니다. 이 스킬은 코드를 쓰지 않습니다 |
| `/pr-check` | 현재 브랜치 PR 의 리뷰 코멘트를 중요도 순으로 정리합니다 |

---

## 브랜치와 커밋

작업은 이슈에서 시작합니다. 이슈 없이 브랜치를 따거나 PR 을 올리지 않습니다.
작업 브랜치는 `dev` 에서 따고 `dev` 로 PR 을 올립니다. `main` 은 배포 가능한
상태만 두며 `dev` 에서만 넘어옵니다. `main` · `dev` 직접 푸시는 막혀 있습니다.

커밋 메시지는 `{타입}({스코프}): {요약}` 입니다.

```
feat(interview): 면접 세션 생성 API 추가
fix(ai): 세션 시작 타임아웃을 90초로 분리
```

스코프는 건드린 도메인(`interview` `report` …)이나 계층(`ai` `storage` …)
이름입니다. 자세한 것은 `docs/01-conventions.md`.
